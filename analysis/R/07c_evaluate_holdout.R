...
paired_difference <- function(errors_r, condition, against, label) {
  ...
  b_mean <- vapply(split(b$rmse, b$subject_id), mean, numeric(1))
  b_mean <- b_mean[a$subject_id]

  diff <- a$rmse - b_mean
  test <- stats::wilcox.test(a$rmse, b_mean, paired = TRUE)

  data.frame(
    replicate = a$replicate[1],
    comparison = label,
    mean_rmse = round(mean(a$rmse), 4),
    mean_reference_rmse = round(mean(b_mean), 4),
...
paired <- do.call(rbind, lapply(REPLICATES, function(r) {
  ...
  do.call(rbind, list(
    paired_difference(e, "fm", "vanilla", "FM vs vanilla"),
    paired_difference(e, "curated", "vanilla", "Curated vs vanilla"),
    paired_difference(e, "fm", sprintf("random_fm_%d", 1:5),
                      "FM vs matched random"),
    paired_difference(e, "curated", sprintf("random_curated_%d", 1:5),
                      "Curated vs matched random"),
    paired_difference(e, "vanilla", "subject_mean",
                      "Vanilla vs subject mean")
  ))
}))
...
save_progress_figure("07_holdout_comparison.png", {
  ...
  e$arm[e$condition == "subject_mean"] <- "reference"
  ...
  graphics::boxplot(rmse ~ condition, data = e, las = 2, xlab = "",
                    ...
                    ## Headroom for the legend, which would otherwise sit on
                    ## top of the first box.
                    ylim = c(min(e$rmse), max(e$rmse) * 1.15),
